...
module.exports = function(robot) {
  ...
  robot.on("github_deployment", function(msg, deployment) {
    ...
    let user = robot.brain.userForId(deployment.user);
    if (!(user.id != null ? user.id.match(/\d+/) : undefined)) {
      user = robot.brain.userByNameOrMention(user.id);
      if (user.id != null) { deployment.user = user.id; }
    }
    ...
    if ([ "heroku", "capistrano" ].includes(deployment.application.provider)) {
      ...
      return msg.send(`Sorry, I can't deploy ${deployment.name}, the provider is unsupported`);
    }
  });
